...
def is_municipal_grievance(email_content: str) -> bool:
    """
    Uses Claude to classify whether an email is a genuine grievance or complaint
    addressed to the Municipal Corporation.
    Returns True if it should be processed, False if it should be ignored.
    """
    if anthropic_client is None:
        logger.error("Anthropic client is not configured.")
        return False

    prompt = f"""You are an email classifier for Municipal Corporation Ludhiana (MCL).

Your job is to decide: Is this email a genuine grievance, complaint, application, or official correspondence addressed to the Municipal Corporation?

Examples that SHOULD be processed:
- Citizens complaining about water supply, drainage, roads, sanitation, streetlights
- RTI applications
- Legal notices or court orders
- Inter-department official letters
- Building plan or license applications
- Complaints about encroachment, stray animals, garbage

Examples that should be IGNORED (return false):
- Promotional or marketing emails
- Newsletter subscriptions
- Job alerts or recruitment spam
- Social media notifications
- OTP or transaction alerts
- Personal or family messages unrelated to MCL
- Automated system emails (GitHub, Google, etc.)
- Any email not related to municipal services or governance
...
{email_content}"""
    ...
    try:
        ...
        logger.info(f"Email classifier result: {answer}")
        return answer == "true"
    except Exception as e:
        logger.exception(f"Classifier failed: {e}")
        return False  # On failure, skip the email safely
